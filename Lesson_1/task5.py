def bank_deposit_calc(deposit, term, rep):
    percent = 0
    ...
    result = deposit
    for _ in range(term):
        res = rep + (result + rep)*percent / 1200
        result += res
...
rep = int(input('Введите пополнение вклада каждый месяц: '))

print(bank_deposit_calc(dep, term, rep))